...
type Section = "Dashboard" | "Employees" | "Departments" | "Roles" | "Audit";
...
const sectionPermission: Record<Section, string | null> = { Dashboard: null, Employees: "employee.read", Departments: "employee.read", Roles: "role.manage", Audit: "audit.read" };
...
export default function AdminShell() {
  ...
  const loadSection = useCallback(async () => {
    ...
    const paths = { Employees: "/employees", Departments: "/departments", Roles: "/roles", Audit: "/audit-events" };
    setLoading(true); setError(""); try { setItems(await api(paths[section]) as unknown[]); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to load data."); } finally { setLoading(false); }
  }, [api, me, section, token]);
  ...
  return <main className="shell"><aside><h1>DWCO Admin</h1><nav aria-label="Administration">{visibleSections.map(name => <button className={section === name ? "active" : ""} onClick={() => setSection(name)} key={name}>{name}</button>)}</nav></aside><section className="content"><header><div><strong>{section}</strong><span>{me.email}</span></div><button onClick={async () => { if (refreshToken) await api("/auth/logout", { method: "POST", body: JSON.stringify({ refresh_token: refreshToken }) }).catch(() => null); setToken(null); setRefreshToken(null); setMe(null); setItems([]); }}>Sign out</button></header>{error && <p role="alert" className="error banner">{error}</p>}{loading ? <p className="state">Loading…</p> : <Workspace section={section} items={items} can={can} create={create} api={api} reload={loadSection} />}</section></main>;
}
